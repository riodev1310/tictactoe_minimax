...
def check_win(player, check_board=board):
    for col in range(BOARD_COLS): # dọc
        if check_board[0][col] == player and check_board[1][col] == player and check_board[2][col] == player:
            return True
    
    for row in range(BOARD_ROWS): # ngang 
        ...
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLS):
                if available_square(row, col):
                    ...
                    # best_score = min(score, best_score) # Ngu hơn
                    best_score = max(score, best_score) # Khôn hơn
        ...
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLS):
                if available_square(row, col):
                    ...
                    minimax_board[row][col] = 0
                    # best_score = max(score, best_score) # Ngu hơn
                    best_score = min(score, best_score) # Khôn hơn
